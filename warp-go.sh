...
E[3]="This project is designed to add WARP network interface for VPS, using warp-go core, using various interfaces of CloudFlare-WARP, integrated wireguard-go, can completely replace WGCF. Save Hong Kong, Toronto and other VPS, can also get WARP IP. Thanks again @CoiaPrant and his team. Project address: https://github.com/ProjectWARP/warp-go/-/tree/master/"
C[3]="本项目专为 VPS 添加 WARP 网络接口，使用 wire-go 核心程序，利用CloudFlare-WARP 的各类接口，集成 wireguard-go，可以完全替代 WGCF。 救活了香港、多伦多等 VPS 也可以获取 WARP IP。再次感谢 @CoiaPrant 及其团队。项目地址: https://github.com/ProjectWARP/warp-go/-/tree/master/"
E[4]="Choose:"
C[4]="请选择:"
E[5]="You must run the script as root. You can type sudo -i and then download and run it again. Feedback:[https://github.com/james986g/warp-sh/issues]"
C[5]="必须以root方式运行脚本，可以输入 sudo -i 后重新下载运行，问题反馈:[https://github.com/james986g/warp-sh/issues]"
E[6]="This script only supports Debian, Ubuntu, CentOS, Arch or Alpine systems, Feedback: [https://github.com/james986g/warp-sh/issues]"
C[6]="本脚本只支持 Debian、Ubuntu、CentOS、Arch 或 Alpine 系统,问题反馈:[https://github.com/james986g/warp-sh/issues]"
E[7]="Curren operating system is \$SYS.\\\n The system lower than \$SYSTEM \${MAJOR[int]} is not supported. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[7]="当前操作是 \$SYS\\\n 不支持 \$SYSTEM \${MAJOR[int]} 以下系统,问题反馈:[https://github.com/james986g/warp-sh/issues]"
E[8]="Install dependence-list:"
C[8]="安装依赖列表:"
E[9]="Step 3/3: Best MTU and Endpoint found."
C[9]="进度 3/3: 已找到最佳 MTU 和 Endpoint"
E[10]="No suitable solution was found for modifying the warp-go configuration file warp.conf and the script aborted. When you see this message, please send feedback on the bug to:[https://github.com/james986g/warp-sh/issues]"
C[10]="没有找到适合的方案用于修改 warp-go 配置文件 warp.conf，脚本中止。当你看到此信息，请把该 bug 反馈至:[https://github.com/james986g/warp-sh/issues]"
...
E[23]="Failed more than \${j} times, script aborted. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[23]="失败已超过\${j}次，脚本中止，问题反馈:[https://github.com/james986g/warp-sh/issues]"
E[24]="non-"
C[24]="非"
...
C[35]="检查环境中……"
E[36]="The TUN module is not loaded. You should turn it on in the control panel. Ask the supplier for more help. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[36]="没有加载 TUN 模块，请在管理后台开启或联系供应商了解如何开启，问题反馈:[https://github.com/james986g/warp-sh/issues]"
E[37]="Curren architecture \$(uname -m) is not supported. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[37]="当前架构 \$(uname -m) 暂不支持,问题反馈:[https://github.com/james986g/warp-sh/issues]"
...
C[49]="1. 变更为 free 账户\n 2. 变更为另一个 WARP+ 账户\n 3. 变更为 Teams 账户 (可通过 https://web--public--warp-team-api--coia-mfs4.code.run 轻松获取，如果留空，则使用脚本提供的)\n 0. 返回主菜单"
E[50]="Registration of WARP\${k} account failed, script aborted. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[50]="注册 WARP\${k} 账户失败，脚本中止，问题反馈: [https://github.com/james986g/warp-sh/issues]"
E[51]="Warp-go not yet installed. No account registered. Script aborted. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[51]="warp-go 还没有安装，没有注册账户，脚本中止，问题反馈: [https://github.com/james986g/warp-sh/issues]"
E[52]="Wireguard configuration file: /opt/warp-go/wgcf.conf\n"
C[52]="Wireguard 配置文件: /opt/warp-go/wgcf.conf\n"
E[53]="Warp-go installed. Script aborted. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[53]="warp-go 已安装，脚本中止，问题反馈: [https://github.com/james986g/warp-sh/issues]"
...
C[55]="请选择优先级别:\n  1. IPv4\n  2. IPv6\n  3. 使用 VPS 初始设置 (默认)"
E[56]="Download warp-go zip file unsuccessful. Script exits. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[56]="下载 warp-go 压缩文件不成功，脚本退出，问题反馈: [https://github.com/james986g/warp-sh/issues]"
E[57]="Warp-go file does not exist, script exits. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[57]="Warp-go 文件不存在，脚本退出，问题反馈: [https://github.com/james986g/warp-sh/issues]"
...
E[63]="Warp-go installation failed. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[63]="warp-go 安装失败，问题反馈: [https://github.com/james986g/warp-sh/issues]"
...
E[103]="Unlimited"
C[103]="无限制"
E[104]="Failed to get the registration information from API. Script exits. Feedback: [https://github.com/james986g/warp-sh/issues]"
C[104]="API 获取不到注册信息，脚本退出，问题反馈: [https://github.com/james986g/warp-sh/issues]"
...
statistics_of_run-times() {
  local COUNT=$(curl --retry 2 -ksm2 "https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fraw.githubusercontent.com%2Fjames986g%2Fwarp%2Fmain%2Fwarp-go.sh&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=hits&edge_flat=false" 2>&1 | grep -m1 -oE "[0-9]+[ ]+/[ ]+[0-9]+") &&
...
check_cdn() {
  ...
  for CDN in "${RANDOM_CDN[@]}"; do
    wget -T2 -qO- https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh | grep -q '#!/usr/bin/env' && break || unset CDN
  done
}
...
check_install() {
  if [ -s /opt/warp-go/warp.conf ]; then
    ...
    STATUS=0
    {
      # 预下载 warp-go，并添加执行权限，如因 github 接口问题未能获取，默认 v1.0.8
      latest=$(wget -qO- -T1 -t1 https://github.com/api/v4/projects/ProjectWARP%2Fwarp-go/releases | awk -F '"' '{for (i=0; i<NF; i++) if ($i=="tag_name") {print $(i+2); exit}}' | sed "s/v//")
      latest=${latest:-'1.0.8'}
      wget --no-check-certificate -qO /tmp/warp-go.tar.gz https://${CDN}github.com/james986g/intel-test/-/raw/main/warp-go/warp-go_"$latest"_linux_"$ARCHITECTURE".tar.gz
...
change_ip() {
  ...
  warp_restart() {
    ...
    bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh) --file /opt/warp-go/warp.conf.tmp1 --cancle >/dev/null 2>&1
...
uninstall() {
  ...
  [ -s /etc/wireguard/warp_unlock.sh ] && bash <(curl -sSL https://${CDN}github.com/james986g/warp_unlock/-/raw/main/unlock.sh) -U -$L

  # 卸载
  systemctl disable --now warp-go >/dev/null 2>&1
  kill -15 $(pgrep warp-go) >/dev/null 2>&1
  bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh) --file /opt/warp-go/warp.conf --cancle >/dev/null 2>&1
...
ver() {
  ...
  wget -O /tmp/warp-go.sh https://${CDN}github.com/james986g/intel-test/-/raw/main/warp-go.sh
...
registe_api() {
  ...
  until [ -s /opt/warp-go/$REGISTE_FILE ]; do
    ...
    if ! grep -sq 'PrivateKey' /opt/warp-go/$REGISTE_FILE; then
      ...
      CF_API_REGISTE="$(bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh | sed 's# > $registe_path##g; /cat $registe_path/d') --registe --token $TOKEN 2>/dev/null)"
...
EOF
    ...
    if grep -sq 'Account' /opt/warp-go/$REGISTE_FILE; then
      ...
      if [ -n "$LICENSE" ]; then
        local RESULT=$(bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --license $LICENSE)
        if [[ "$RESULT" =~ '"warp_plus": true' ]]; then
          bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --name $NAME >/dev/null 2>&1
          echo "$LICENSE" > /opt/warp-go/License
          echo "$NAME" > /opt/warp-go/Device_Name
        ...
        if [ -s /opt/warp-go/warp.conf.tmp ]; then
          bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --license $(cat /opt/warp-go/License 2>/dev/null) >/dev/null 2>&1
          bash <(curl -m8 -sSL https://${CDN}github.com/james986g/intel-test/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --name $(cat /opt/warp-go/Device_Name 2>/dev/null) >/dev/null 2>&1
        fi
      fi
    else
      rm -f /opt/warp-go/$REGISTE_FILE
...
install() {
  ...
  # 寻找最佳 MTU 和 Endpoint 值
  {
    ...
    wget $STACK -qO /tmp/endpoint https://${CDN}github.com/james986g/intel-test/-/raw/main/endpoint/warp-linux-${ARCHITECTURE//amd64*/amd64} && chmod +x /tmp/endpoint
    [ "$IPV4$IPV6" = 01 ] && wget $STACK -qO /tmp/ip https://${CDN}github.com/james986g/intel-test/-/raw/main/endpoint/ipv6 || wget $STACK -qO /tmp/ip https://${CDN}github.com/james986g/intel-test/-/raw/main/endpoint/ipv4
...
Documentation=https://github.com/james986g/intel-test-sh
Documentation=https://github.com/ProjectWARP/warp-go
