...
check_cdn() {
  ...
  for CDN in "${RANDOM_CDN[@]}"; do
    wget -T2 -qO- https://${CDN}github.com/james986g/warp/-/raw/main/api.sh | grep -q '#!/usr/bin/env' && break || unset CDN
  done
}
...
check_install() {
  if [ -s /opt/warp-go/warp.conf ]; then
    ...
    STATUS=0
    {
      ...
      latest=${latest:-'1.0.8'}
      wget --no-check-certificate -qO /tmp/warp-go.tar.gz https://${CDN}github.com/james986g/warp/-/raw/main/warp-go/warp-go_"$latest"_linux_"$ARCHITECTURE".tar.gz
...
change_ip() {
  ...
  warp_restart() {
    ...
    bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh) --file /opt/warp-go/warp.conf.tmp1 --cancle >/dev/null 2>&1
...
uninstall() {
  ...
  bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh) --file /opt/warp-go/warp.conf --cancle >/dev/null 2>&1
...
ver() {
  ...
  wget -O /tmp/warp-go.sh https://${CDN}github.com/james986g/warp/-/raw/main/warp-go.sh
...
registe_api() {
  ...
  until [ -s /opt/warp-go/$REGISTE_FILE ]; do
    ...
    if ! grep -sq 'PrivateKey' /opt/warp-go/$REGISTE_FILE; then
      ...
      CF_API_REGISTE="$(bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh | sed 's# > $registe_path##g; /cat $registe_path/d') --registe --token $TOKEN 2>/dev/null)"
...
EOF
    ...
    if grep -sq 'Account' /opt/warp-go/$REGISTE_FILE; then
      ...
      if [ -n "$LICENSE" ]; then
        local RESULT=$(bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --license $LICENSE)
        if [[ "$RESULT" =~ '"warp_plus": true' ]]; then
          bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --name $NAME >/dev/null 2>&1
          echo "$LICENSE" > /opt/warp-go/License
          echo "$NAME" > /opt/warp-go/Device_Name
        ...
        if [ -s /opt/warp-go/warp.conf.tmp ]; then
          bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --license $(cat /opt/warp-go/License 2>/dev/null) >/dev/null 2>&1
          bash <(curl -m8 -sSL https://${CDN}github.com/james986g/warp/-/raw/main/api.sh) --file /opt/warp-go/$REGISTE_FILE --name $(cat /opt/warp-go/Device_Name 2>/dev/null) >/dev/null 2>&1
        fi
      fi
    else
      rm -f /opt/warp-go/$REGISTE_FILE
...
install() {
  ...
  # 寻找最佳 MTU 和 Endpoint 值
  {
    ...
    wget $STACK -qO /tmp/endpoint https://${CDN}github.com/james986g/warp/-/raw/main/endpoint/warp-linux-${ARCHITECTURE//amd64*/amd64} && chmod +x /tmp/endpoint
    [ "$IPV4$IPV6" = 01 ] && wget $STACK -qO /tmp/ip https://${CDN}github.com/james986g/warp/-/raw/main/endpoint/ipv6 || wget $STACK -qO /tmp/ip https://${CDN}github.com/james986g/warp/-/raw/main/endpoint/ipv4
...
Documentation=https://github.com/james986g/warp-sh
